...
4. Once AWS credentials are wired up (see `DEPLOY.md`), validate against the real deployed stack the same way the original project validated against its Docker Compose stack — don't skip that step once it's available.
...
2. ~~**Auth**~~ — `register`/`login` Lambdas, `Users` table, JWT issuing. *(done)*
3. ~~**WebSocket connect/disconnect**~~ — `$connect`/`$disconnect` Lambdas, `Connections` table. *(done)*
4. ~~**Send message**~~ — `sendMessage` Lambda: write to `Messages`, fan out via `postToConnection`. *(done)*
5. ~~**REST conversations/users routes**~~ — `getConversations`, `getMessages`, `createConversation`, `updateMe`, `markRead`. *(done)*
6. ~~**Client rewiring**~~ — native `WebSocket` in place of `socket.io-client`; `api.ts`/raw `fetch` calls point at `VITE_API_URL`. *(done)*
7. **Deploy pipeline** — `cdk deploy` on merge to `main`; static client build to S3 + CloudFront. See `DEPLOY.md` for the one-time AWS/OIDC setup this needs before it can run.
