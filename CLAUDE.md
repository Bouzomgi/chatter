...
After implementing a feature: commit the work, push the branch, open a PR, then loop (`/loop`) monitoring CI until all checks pass. Before declaring a PR ready, validate every item in the PR test plan by running the Playwright E2E tests against the app (`PLAYWRIGHT_BASE_URL=http://localhost:5173 npx playwright test`). Check off each passing item in the PR description using `gh pr edit`. Fix any failures before stopping.
