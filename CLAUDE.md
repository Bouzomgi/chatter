...
4. Rebuild the Docker stack so the user can visually confirm the feature. The client SPA is served by nginx; only rebuild `red` when server code changed:
   - Frontend-only change: `docker compose build nginx 2>&1 | tail -5 && docker compose up -d`
   - Backend or full-stack change: `docker compose build red nginx 2>&1 | tail -5 && docker compose up -d`
   - Only use `--no-cache` when dependencies (package.json/pnpm-lock.yaml) changed — otherwise omit it.
   - The stack runs at `http://localhost:80`; admin credentials are `admin@admin.local` / `admin123`.
